...
const double SCORE_DY = -25;           /* adjustment for score label text    */
...
const int SCORE_FONT_SIZE = 12;
const int WORD_FONT_SIZE = 10;
const int SCOREBOX_WIDTH = 100;

const int PLAYER_NUMBER = sizeof(Player);
...
static GWindow *gwp;
static GLabel *playerScoreBox[PLAYER_NUMBER];
...
void drawBoard(int numRows, int numCols) {
   ...
   for (int player = HUMAN; player <= COMPUTER; player++)
   {
       playerScoreBox[player]->setLocation(gState.scoreBox[player].x
                      + gState.scoreBox[player].w - playerScoreBox[player]->getWidth(),
                    gState.scoreBox[player].y + SCORE_DY);
       playerScoreBox[player]->setLabel("");
   }
 }
 ...
 */
 ...
 //     if ((int) letters.length() < gState.numRows * gState.numCols) {
 ...
 //         col++;
 //         if (col == gState.numCols) {
 //             col = 0;
 //             row++;
 //             if (row == gState.numCols) {
 //                 labeledAll = true;
 //                 break;
 //             }
 //         }
...
static void drawCube(int row, int col, char ch, bool invert) {
   ...
   drawCenteredChar(cubeX(col) + gState.cubeSize * 1.25,
                    cubeY(row) + gState.cubeSize/1.8, ch,
                    invert ? DIE_COLOR : LETTER_COLOR);
}
...
static void addToScoreForPlayer(int pointsToAdd, Player player) {

    playerScoreBox[player]->setLabel(integerToString(gState.scores[player] += pointsToAdd));
...
void recordWordForPlayer(string word, Player player) {
   ...
   label.setLocation(gState.scoreBox[player].x + col * gState.wordColumnWidth + 10,
                     gState.scoreBox[player].y + (row + 1) * WORD_FONT_SIZE * 1.2
                                               + WORD_DY + 5);
   gwp->add(label);
...
/*
 * Handy utility functions to return coordinates of the upper-left
...
void initializeScoreBox()
{
    for (int player = HUMAN; player <= COMPUTER; player++) {
        playerScoreBox[player] = new GLabel();
        playerScoreBox[player]->setWidth(SCOREBOX_WIDTH);
        playerScoreBox[player]->setFont(SCORE_FONT + "-" + integerToString(SCORE_FONT_SIZE));
        playerScoreBox[player]->setColor(LABEL_COLOR);
        gwp->add(playerScoreBox[player]);
    }
 }
...
static void drawCenteredChar(double xc, double yc, char ch, string color) {
   ...
   //label.setFont(CUBE_FONT + "-" + integerToString(int(gState.fontSize)));
   label.setFont(CUBE_FONT + "-" + integerToString(int(gState.fontSize * 2)) + "-Bold");

   label.setLocation(xc - label.getWidth() / 2, yc + 0.5 * gState.fontSize);
   gwp->add(label);
}
...
void reset() {
    ...
    // Reset scores and word lists for both players
    for (int player = HUMAN; player <= COMPUTER; player++) {
        gState.scores[player] = 0;
        gState.numWords[player] = 0;
        playerScoreBox[player]->setLabel("");
    }

    // Redraw the empty board with the existing number of rows and columns
    drawBoard(gState.numRows, gState.numCols);
